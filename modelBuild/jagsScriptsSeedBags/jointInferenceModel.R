model { 
  ...
  for(i in 1:n2){
    ...
    # likelihood
    totalJan2[i] ~ dbinom(theta_4[i]*test[siteBags2[i],yearBags2[i]], seedStart2[i])
    ...
    y_total2[i] ~ dbinom(theta_4[i]*test[siteBags2[i],yearBags2[i]], seedStart2[i])
  ...
  for(i in 1:n_siteBags){
    ...
    p.i_e[i] ~ dnorm(mu0_e[i],tau0_e[i])
    logit(p_e[i]) <- p.i_e[i]
