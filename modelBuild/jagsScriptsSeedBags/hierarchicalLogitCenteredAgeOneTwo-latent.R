...
# seed bags ---------------------------------------------------------------
  ...
  for(i in 1:n_siteBags){
    ...
    nu_1c[i] = (nu_1[i]+(nu_2[i])^(1/2))/2

    s1[i] = p_1[i]*(p_2[i] + (1-p_2[i])*(nu_1c[i])^(1/3))
    g1[i] = p_2[i]/(1-(1-(nu_1c[i]^(1/3)))*(1-p_2[i]))
    s2[i] = p_3[i]*(nu_1c[i]^(2/3))
    ...
    for(k in 1:n_yearBags){
      ...
      nu0_1c[i,k] = (nu0_1[i,k]+(nu_2[i])^(1/2))/2
  
      s1.0[i,k] = p0_1[i,k]*(p0_2[i,k] + (1-p0_2[i,k])*(nu0_1c[i,k])^(1/3))
      g1.0[i,k] = p0_2[i,k]/(1-(1-(nu0_1c[i,k]^(1/3)))*(1-p0_2[i,k]))
      s2.0[i,k] = p0_3[i,k]*(nu0_1c[i,k]^(2/3))
  ...
  for(i in 1:n_siteBags2){
    ...
    nu_2c[i] = ((nu_1[i]+(nu_2[i])^(1/2))/2)^1/2
    nu_2cc[i] = (nu_1[i]^(2/3))*(nu_2c[i]^(1/3))
    
    s3[i] = p_4[i]*(p_5[i] + (1-p_5[i])*(nu_2cc[i]))
    g2[i] = p_5[i]/(1-(1-(nu_2c[i]^(1/3)))*(1-p_5[i]))
    s4[i] = p_6[i]*(nu_2c[i]^(2/3))
    ...
    for(k in 1:n_yearBags2){
      ...
      nu0_2c[i,k] = ((nu0_1[i,k]+(nu0_2[i,k])^(1/2))/2)^1/2
      nu0_2cc[i,k] = (nu0_1[i,k]^(2/3))*(nu0_2c[i,k]^(1/3))
      
      s3.0[i,k] = p0_4[i,k]*(p0_5[i,k] + (1-p0_5[i,k])*(nu0_2cc[i,k]))
      g2.0[i,k] = p0_5[i,k]/(1-(1-(nu0_2c[i,k]^(1/3)))*(1-p0_5[i,k]))
      s4.0[i,k] = p0_6[i,k]*(nu0_2c[i,k]^(2/3))
