...
dirFigures = c("/Users/Gregor/Dropbox/clarkiaSeedBanks/products/figures2/")
...
mcmcSummary<-mcmcSamples %>%
  ...
  dplyr::summarise(med = median(nu_1), 
                   ci.lo = quantile(nu_1,probs=0.025), 
                   ci.hi = quantile(nu_1,probs=0.975),
                   ci.lo2 = quantile(nu_1,probs=0.25), 
                   ci.hi2 = quantile(nu_1,probs=0.75)
  )
mcmcSummary<-cbind(mcmcSummary,site=siteNames)

nu_1Summary <- mcmcSummary %>%
...
saveRDS(g1Summary,file=paste0(fileDirectory,"nu_1Summary.RDS"))
