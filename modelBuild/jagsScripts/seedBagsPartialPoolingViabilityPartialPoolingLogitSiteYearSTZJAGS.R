...
model { 
    ...
    for(i in 1:nsites){
        alpha.i[i] ~ dnorm(0, 0.0001)
        for(k in 1:nyears){
            beta.0.i[i,k] ~ dnorm(0, 0.0001)
        }
    }
    
    # sum to zero constraint
    for(i in 1:nsites){
        beta.i[i,1] <- 0 - sum( beta.0.i[i,2:nyears] )
        beta.i[i,2:nyears] <- beta.0.i[i,2:nyears]
    }
    ...
    for(i in 1:N_burial){
        ...
        # s1 seed survival
        pi[i] <- ilogit(alpha.i[site[i]] + beta.i[site[i],year[i]])
