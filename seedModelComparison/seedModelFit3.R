...
lines(seq(0,24,by=0.1),1/((1+mean(MCMCchains(samples.rjags,"b"))*seq(0,24,by=0.1))^mean(MCMCchains(samples.rjags,"a"))))
...
plot(rep(1,length=data$n1),data$seedlingJan/data$seedStart,xlim=c(0,3),ylim=c(0,1))
points(1,mean(data$seedlingJan/data$seedStart),pch=16,cex=2)
abline(h=mean(MCMCchains(samples.rjags,"g1")),lty=2,col=colors[1])

points(rep(2,length=data$n2),data$seedlingJan_3/data$seedStart_3,xlim=c(0,3),ylim=c(0,1))
points(2,mean(data$seedlingJan_3/data$seedStart_3),pch=16,cex=2)
abline(h=mean(MCMCchains(samples.rjags,"g1")),lty=2,col=colors[2])


1/((1+mean(MCMCchains(samples.rjags,"b"))*seq(0,24,by=0.1))^mean(MCMCchains(samples.rjags,"a")))

a=1/((1+mean(MCMCchains(samples.rjags,"b"))*3)^mean(MCMCchains(samples.rjags,"a")))

b=1/((1+mean(MCMCchains(samples.rjags,"b"))*12)^mean(MCMCchains(samples.rjags,"a")))
c=1/((1+mean(MCMCchains(samples.rjags,"b"))*15)^mean(MCMCchains(samples.rjags,"a")))
d=1/((1+mean(MCMCchains(samples.rjags,"b"))*24)^mean(MCMCchains(samples.rjags,"a")))
