model {
  ...
  for(i in 1:n3){
    ...
    mu5[i] <- (1-g[1])*(1-g[2])*exp(-1*(k*months5[i]))
    alpha5[i] <- (mu5[i]*beta[5])/(1-mu5[i])
    theta5[i] ~ dbeta(alpha5[i]+.01,beta[5]+.01)
    
    mu6[i] <- (1-g[1])*(1-g[2])*(1-g[3])*exp(-1*(k*months6[i]))
