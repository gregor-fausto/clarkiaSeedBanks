model {
  ...
  for(i in 1:n1){
    ...
    mu[i] <- (((1-g[siteSurvival[i],germIndex[i]])^(g_1[i]))*((1-g[siteSurvival[i],germIndex[i]])^(g_2[i]))*((1-g[siteSurvival[i],germIndex[i]])^(g_3[i])))*exp(-(months[i]/b[siteSurvival[i]])^a[siteSurvival[i]])
  ...
  for(i in 1:n2){
    # likelihood
    seedlingJan[i] ~ dbinom(g[siteGermination[i],gIndex[i]], totalJan[i])

    # log-likelihood
    logLik_g[i] <- logdensity.bin(seedlingJan[i],g[siteGermination[i],gIndex[i]] ,totalJan[i])
  }
