model {
  ...
  for(i in 1:n_siteSurvival){
    ...
    # WEAKLY INFORMATIVE
    sigma0_s[i] ~ dnorm(0,4) T(0,)
    tau0_s[i] <- 1/(sigma0_s[i]*sigma0_s[i])

    # HALF CAUCHY
    # sigma0_s[i] ~ dt(0,1/2.5^2,1) I(0,)
    # tau0_s[i] <- 1/(sigma0_s[i]*sigma0_s[i])
    ...
    for(j in 1:n_yearSurvival){
      ...
      # WEAKLY INFORMATIVE
      sigma_s[i,j] ~ dnorm(0,4) T(0,)
      tau_s[i,j] <- 1/(sigma_s[i,j]*sigma_s[i,j])

      # HALF CAUCHY
      # sigma_s[i,j] ~ dt(0,1/2.5^2,1) I(0,)
      # tau_s[i,j] <- 1/(sigma_s[i,j]*sigma_s[i,j])
    }
  ...
  for(i in 1:n1){
   ...
   #  mu[i] <- exp(-b[i]*months[i])
    ...
    # PRIOR PREDICTIVE
    #y_prior.pred[i] ~ dbinom(theta_s[i], seedStart[i])
  }
