model {
  ...
  for(j in 1:n_siteBags){
    ...
    for(k in 1:n_ageBags){
      ...
      # theta 1
      mu0_1[j,k] ~  dnorm(0, 0.001)
      sigma0_1[j,k] ~ dnorm(0, 0.3) T(0,)
      tau0_1[j,k] <- 1/(sigma0_1[j,k]*sigma0_1[j,k])
      
      # theta 2
      mu0_2[j,k] ~  dnorm(0, 0.001)
      sigma0_2[j,k] ~ dnorm(0, 0.3) T(0,)
      tau0_2[j,k] <- 1/(sigma0_2[j,k]*sigma0_2[j,k])
      
      # theta 3
      mu0_3[j,k] ~  dnorm(0, 0.001)
      sigma0_3[j,k] ~ dnorm(0, 0.3) T(0,)
      tau0_3[j,k] <- 1/(sigma0_3[j,k]*sigma0_3[j,k])
      
      # theta 4
      mu0_4[j,k] ~  dnorm(0, 0.001)
      sigma0_4[j,k] ~ dnorm(0, 0.3) T(0,)
  ...
  for(i in 1:n){
    ...
    intactOct2[i] ~ dbinom(theta_4[i], seedStart[i])
