...
data = readRDS("~/Dropbox/dataLibrary/workflow/data/belowgroundDataAgeOneTwo-S22.RDS")

#data = readRDS("~/Dropbox/dataLibrary/workflow/data/belowgroundDataAgeOneTwoTesting.RDS")

data$intactOctDup = data$intactOct
# 
# trueVec = data$siteBags==18
# trueVec2 = data$siteBags2==18
# 
# trueVec3 = data$siteViab==18
# trueVec4 = data$siteViab2==18
# 
# for(i in c(1,3,5,7:10,47)){
#   data[[i]]=data[[i]][trueVec]
# }
# for(i in c(12,14,16,18,20:23)){
#   data[[i]]=data[[i]][trueVec3]
# }
# for(i in c(24,26,28,30:33)){
#   data[[i]]=data[[i]][trueVec2]
# }
# for(i in c(34,36,38,40,42:45)){
#   data[[i]]=data[[i]][trueVec4]
# }
# 
# data$n = length(data$siteBags)
# data$n2 = length(data$siteBags2)
data$n_bag  = length(data$bag)
data$n_bag2  = length(data$bag2)
# data$n_siteBags = data$n_siteBags2 = data$n_siteViab = data$n_siteViab2  = 1
...
# # tuning (n.adapt)
jm = jags.model(paste0(dir,"jointInferenceModel.R"), data = data, inits = inits,
...
parsToMonitor = c(paste(rep("mu0",5),c(1:3,"g","v","e"),sep="_"),
                  ...
                  "p_g","p_v","p_g2","p_v2","p_e","test",
                  "nu_1","nu_2","nu_2c",
                  "p_1","p_2","p_3","p_4","p_5",
                  "s1","g1","s2","s3","s4","g2",
                  "theta_e","theta_4","theta_5","theta_6",
                  
                  "p0_g","p0_v","p0_g2","p0_v2","p0_e",
                  "nu0_1","nu0_2","nu0_2c",
                  "p0_1","p0_2","p0_3","p0_4","p0_5",
                  "s1.0","s2.0","s3.0","g1.0","g2.0","s4.0",
                  "y_total","y_seedling","y_october","y_octtot",
...
# 
# saveRDS(samples.rjags,file=paste0(fileDirectory,"jointInferenceSamples.RDS"))

MCMCsummary(samples.rjags,params="y_seedling")

y_seedling <- MCMCchains(samples.rjags,params="y_seedling")
plot(data$seedlingJan,apply(y_seedling,2,mean))
abline(a=0,b=1)
