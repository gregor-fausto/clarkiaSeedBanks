...
initsA <- function(samps = data$n_siteSurvival){
  rgamma(n = samps, shape = 20, rate = 20)
}
...
for(i in 1:3){
  ...
  names(inits[[i]]) = c("beta","mu0_s","sigma0_s","sigma_s",
...
names(temp) = c("beta","mu0_s","sigma0_s","sigma_s",
                "mu0_g", "sigma0_g", "sigma_g",
                "mu0_s0", "sigma0_s0", "sigma_s0",
                ".RNG.name",".RNG.seed")
return(temp)
...
parsToMonitor = c("beta","mu0_s","sigma0_s","sigma_s",
                  "mu0_g", "sigma0_g", "sigma_g",
                  "mu0_s0", "sigma0_s0", "sigma_s0")
...
out <- clusterEvalQ(cl, {
  library(rjags)
  jm = jags.model(file="~/Dropbox/clarkiaSeedBanks/scriptsModelFitting/jagsScripts/jags-seeds-parallel.R",
