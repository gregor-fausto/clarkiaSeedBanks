...
# get MCMC samples
mcmcSamples <- readRDS(mcmcSampleFiles[[grep("seedlingSurvivalSamples-noPool.rds", mcmcSampleFiles)]])
...
f(x = "mu", model = "seedlingSurvival-noPool")
f(x = "sigma", model = "seedlingSurvival-noPool")

# recover chains
all.chains = MCMCchains(mcmcSamples, params = c("mu", "sigma"), mcmc.list = TRUE)
...
# print histogram
jpeg(filename = paste0(outputDirectory, "rhat-heidelberg-seedlingSurvival-noPool", ".jpeg"), 
  quality = 75)
