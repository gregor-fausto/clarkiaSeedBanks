...
var_sigma = rsMedians %>% 
  ...
  dplyr::group_by(site) %>%
  dplyr::summarise(var_sigma = gvar.am(med_sigma))

var_fec = rsMedians %>% 
  dplyr::filter(!is.na(rsMedians)) %>%
  dplyr::group_by(site) %>%
  dplyr::summarise(var_fec = gvar.am(med_fec))

var_phi = rsMedians %>% 
  dplyr::filter(!is.na(rs)) %>%
  dplyr::group_by(site) %>%
  dplyr::summarise(var_phi = gvar.am(med_phi))

cov_sigma.fec = rsMedians %>% 
  dplyr::filter(!is.na(rs)) %>%
  dplyr::group_by(site) %>%
  dplyr::summarise(cov_sigma.fec = gcovar.am(med_sigma,med_fec))

cov_sigma.phi = rsMedians %>% 
  dplyr::filter(!is.na(rs)) %>%
  dplyr::group_by(site) %>%
  dplyr::summarise(cov_sigma.phi = gcovar.am(med_sigma,med_phi))

cov_fec.phi = rsMedians %>% 
  dplyr::filter(!is.na(rs)) %>%
  dplyr::group_by(site) %>% 
  dplyr::summarise(cov_fec.phi = gcovar.am(med_fec,med_phi))
...
variancePlot <- varianceComponents %>%
  ...
  geom_hline(yintercept=c(exp(0),exp(1),exp(2)),linetype='dotted') +
  geom_point() +
