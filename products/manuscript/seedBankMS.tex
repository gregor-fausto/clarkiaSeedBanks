...
I fit the following model for bag $i$ in year $j$ in population $k$. 
%
\begin{align}
  \begin{split}
...
\end{align}
%
From the model above, we obtain $\theta_1$ for each population by marginalizing across years and taking the inverse logit.
...
\end{align}
%
From the model above, we obtain $\theta_2$ for each population by marginalizing across years and taking the inverse logit.
...
\end{align}
%
From the model above, we obtain $\theta_3$ for each population by marginalizing across years and taking the inverse logit.

I used the estimates from the model to derive transition probabilities as described in \textbf{Appendix on Conditional Probability}:
%
    \begin{align}
\begin{split}
...
To estimate the survival and germination of seeds in each population, I fit the following model for bag $i$ at population $j$ in year $k$:
%
\begin{align}
  \begin{split}
...
%
\begin{align}
\mathrm{f}_1(\alpha^{s1}_{0,j} , \beta^{s1}_{jk} ) ) & =  \phi_{jk} = \mathrm{logit}^{-1}(\alpha^{s1}_{0,j} + \beta^{s1}_{jk}) \label{eq:s1} \\
\mathrm{f}_2(\alpha^{g1}_{0,j} , \beta^{g1}_{jk},  \alpha_{ijk} ) ) & = \gamma_{jk} \times v_{ijk}^\frac{1}{3} = \mathrm{logit}^{-1}(\alpha^{s1}_{0,j} + \beta^{s1}_{jk}) \times ( \mathrm{logit}^{-1}(\alpha_{ijk}))^\frac{1}{3}  \label{eq:g1} \\
\mathrm{f}_3(\alpha^{s2}_{0,j} , \beta^{s2}_{jk} ) ) & =  \rho_{jk} = \mathrm{logit}^{-1}(\alpha^{s1}_{0,j} + \beta^{s1}_{jk})  \label{eq:s2}
\end{align}
