...
plotList <- list()
for(i in 1:20){
plotList[[i]]<-(countFruitsPerPlantAllPlots %>%
  dplyr::filter(year>2006) %>%
  dplyr::filter(site==siteNames[i]) %>%
  ggplot( aes(x=countFruitNumberPerPlant, fill=as.factor(permanentPlot))) +
...
I looked at the data in a different way, plotting just the difference between the full and permanent plot data.
