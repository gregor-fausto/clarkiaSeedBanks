model {
  ...
  #   mu[i] <- theta_c[siteSurvival[i],compIndex[i]]*exp(-b[i]*months[i])

    # WEIBULL MODEL
    eta_surv[i] ~ dnorm(mu_s[siteSurvival[i],yearSurvival[i]],tau_s[siteSurvival[i],yearSurvival[i]])
    inv.b[i] <- exp(-(eta_surv[i])/(a[siteSurvival[i]]+.01))
    # model without germination pieces
  ...
  #     # model without germination pieces
  #     mu_survival.pred[i] <- exp(-(months_pred[i]/i.b[i])^a[siteSurvival_pred[i]])
  #     mu.pred[i] <- theta_c[siteSurvival_pred[i],compIndex_pred[i]]*exp(-(months_pred[i]/i.b[i])^a[siteSurvival_pred[i]])
