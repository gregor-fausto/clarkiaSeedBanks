...
jmWrAg = jags.model(paste0(dir,"binomialLikelihood-logLink-expDecay-1.R"), data = data, n.adapt = n.adapt,
                    n.chains=2)
...
## Priors on lambda
b=MCMCchains(samples.rjags,params="b")
par(mfrow=c(2,3))
for(i in 1:6){
  hist.chain(b[,i])
}
...
#year1
plot(data$months[index],y_prior.pred[1,index],type='n',ylim=c(0,100))
for(i in 1:length(samples)){
  points(data$months[index[1:6]]+rnorm(1,0,sd=.01),
         y_prior.pred[samples[i],index[1:6]],
         pch=16,cex=.5)
}
...
plot(data$months[index],y_prior.pred[1,index],type='n',ylim=c(0,100))
for(i in 1:length(samples)){
  points(data$months[index[7:10]]+rnorm(1,0,sd=.01),
         y_prior.pred[samples[i],index[7:10]],
         pch=16,cex=.5)
}
...
plot(data$months[index],y_prior.pred[1,index],type='n',ylim=c(0,100))
for(i in 1:length(samples)){
  points(data$months[index[11:12]]+rnorm(1,0,sd=.01),
         y_prior.pred[samples[i],index[11:12]],
         pch=16,cex=.5)
}
...
#   # }
#   
#   ind=dim(MCMCchains(samples.rjagsWrAg,params="mu"))[2]
#   par(mfrow=c(2,1))
